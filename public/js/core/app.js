...
function NXBootCards() {
    ...
    $("#task-due-date-container").pDatepicker({
        format: 'YYYY-MM-DD',
        autoClose: true,
        initialValue: false
    });
    ...
    // Enable x-editable
    // $('#task-due-date-container').editable({
    //     format: 'YYYY-MM-DD',
    ...
    //         // PersianDatePicker returns the selected date in newValue
    //         console.log('Updated value: ' + newValue);
    //
    //         // Send data to the server after picking a date
    //         // You can make an AJAX request here to submit the data
    //         // Example:
    //         // $.ajax({
    //         //   type: 'POST',
    //         //   url: '/save.php',
    //         //   data: { id: 1, date: newValue },
    ...
    if ($("#cardModal").length) {
        ...
        //reset picker
        // $('.card-pickadate').persianDatepicker('destroy');

        $('.card-pickadate').persianDatepicker({
            observer: true,
            format: 'YYYY/MM/DD',
            autoClose: true,
            initialValue: true,
            ...
            onSelect: function(unix){
                ...
                const elm = $(this)[0].model.inputElement;
                $("#" + $(elm).attr('data-hidden-field')).val(moment(unix).format('YYYY-MM-DD'));
                //update value
                $("#" + $(elm).attr('data-container')).html(
                    new persianDate(unix).format('YYYY/MM/DD')
                );
                //send request
                nxAjaxUxRequest($(elm));
            }
        ...
        $(document).find("._pickadate").persianDatepicker({
...
function NXremindersDatePicker() {
    ...
    // $('#reminders-datetimepicker').persianDatepicker({
    ...
    $('#reminders-datetimepicker').persianDatepicker({
        ...
        onSelect: function () {
            ...
            var value = pd.parse($("#reminders-datetimepicker").val());
            var jdf = new jDateFunctions();
            // $('#converted').text(jdf.getGDate(value));
            $('#reminder_datetime').val(jdf.getGDate(value));

        }
    });

    // $('#reminders-datetimepicker').on('change.persianDatepicker', function (event) {
