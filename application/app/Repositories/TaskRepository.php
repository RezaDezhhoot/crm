...
class TaskRepository {
    ...
    public function search($id = '', $data = []) {
        ...
        if ($data['apply_filters']) {
            ...
            //filter milestone id
            if (request()->query('filter_task_milestoneid')) {

                $milestoneid = array_value_recursive('milestone_id',Milestone::query()->with('child')->find(request('filter_task_milestoneid'))->toArray());
                $tasks->whereIn('task_milestoneid', $milestoneid);
            }
...
//            stats: - counting
             ...
             if (request()->filled('filter_my_tasks')) {
                 if (! auth()->user()->is_admin) {
                     $tasks->whereHas('assigned', function ($query) {
                         ...
                         return $q->whereHas('supervisors',function ($q) {
                             return $q->whereIn('user_id',[auth()->id()]);
                         });
                     });                            
                 }
    ...
    public function create($position = '') {
        ...
        $task->task_date_due = request()->filled('task_date_due') ? ChangeDateBeforeValidation(request('task_date_due')) : null;
