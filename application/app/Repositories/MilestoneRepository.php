...
class MilestoneRepository {
     ...
     */
    public function search($id = '' , $tree = false , $ignore = [] , $paginate = false) {

        //new query
        $milestones = $this->milestone->newQuery();

        $milestones->when($tree , function ($q){
        ...
        $milestones->when(sizeof($ignore) > 0 , function ($q) use ($ignore) {
            return $q->whereNotIn('milestone_id',$ignore);
        });
        ...
        return $milestones->paginate($paginate ? $paginate : null);
    }
    ...
    public function autocompleteNames($type = '', $searchterm = '') {
        ...
        $query->selectRaw("CONCAT_WS('@', milestone_title , milestone_id) AS value, milestone_id as id");
        ...
        $query->whereRaw("milestone_title LIKE '%$searchterm%'");
    ...
    public function create() {
        ...
        $milestone->parent_id = request('parent_id');
