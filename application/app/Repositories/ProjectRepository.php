...
class ProjectRepository {
     ...
     */
    public function search($id = '', $data = [] , $tree = false , $ignore = []) {
        ...
        $projects->when($tree , function ($q){
           return $q->with(['child','parent','client'])->whereNull('parent_id');
        });
        ...
        if ($data['apply_filters']) {
            ...
            //stats - count child
            if (isset($data['stats']) && $data['stats'] == 'count-child') {
                $projects->whereNotNull('parent_id');
            }
    ...
    public function create() {
        ...
        $project->project_date_start = ChangeDateBeforeValidation(request('project_date_start'));
        $project->is_locked = \request('is_locked',false);
        $project->project_date_due = \request()->filled('project_date_due') ? ChangeDateBeforeValidation(request('project_date_due')) : null;
    ...
    public function update($id) {
      //  dd(request('project_date_start'));
        ...
        $project->parent_id = request('parent_id');
        $project->is_locked = \request('is_locked') == 'on' ? true : false;
        ...
        $project->supervisors()->sync(\request('supervisor'));
    ...
    public function autocompleteFeed2($status = '', $limit = '', $searchterm = '') {
        ...
        //start
        $query = $this->projects->newQuery();
        $query->selectRaw("CONCAT_WS('@', project_title , project_id) AS value, project_id AS id");
