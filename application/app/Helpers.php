...
function ChangeDateBeforeValidation($time = null)
{
    if (! is_null($time)) {
        $v = Verta::parse($time)->datetime();
...
function ChangeDateBeforeValidationReminder($time = null)
{
    if (! is_null($time)) {
        $v = Verta::parse($time)->datetime();
        return $v->format('Y-m-d');
...
function convert_to_jalali($value){
...
function GetCountSubCategory($category_id)
{
    return  \App\Models\Project::where('parent_id',$category_id)->get()->count();
}
/**
 * bootstrap class, based on value
 * @param string value the status of the task
