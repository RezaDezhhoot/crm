        ...
        if (request('show_after_adding') == 'on') {
            ...
            if ($count == 1) {
                ...
                if (auth()->user()->pref_view_projects_layout == 'list') {
                    $html = view('pages/projects/views/list/table/tree', compact('projects'))->render();
                    $jsondata['dom_html'][] = array(
                        'selector' => '#projects-td-container'.($project->parent_id ?? null),
                        'action' => 'prepend',
                        'value' => $html);
                }
                ...
                if (auth()->user()->pref_view_projects_layout == 'card') {
                    $html = view('pages/projects/views/cards/layout/tree', compact('projects'))->render();
                    $jsondata['dom_html'][] = array(
