...
use App\Repositories\ProjectSupervisorRepository;
...
class Projects extends Controller {
    ...
    public function index(CategoryRepository $categoryrepo) {
        ...
        //get team projects
        $projects = $this->projectrepo->search(tree: true);

        //apply some permissions
        if ($projects) {
            $this->applyProjectPermissions($projects);
        }
    ...
    private function applyProjectPermissions($projects)
    {
        foreach ($projects as $project) {
            $this->applyPermissions($project);
            if ($project->children) {
                $this->applyProjectPermissions($project->children);
            }
        }
    ...
    public function store(
        ...
        ProjectSupervisorRepository $supervisorrepo,
...
//        $supervisorrepo->add($project_id);
        ...
        $project->supervisors()->attach(\request('supervisor'));
     ...
     * @return \Illuminate\Http\Response
     */
    public function update(ProjectValidation $request, ProjectAssignedRepository $assignedrepo,ProjectSupervisorRepository $supervisorrepo, ProjectManagerRepository $managerrepo, $id) {
        //get project
        $projects = $this->projectrepo->search($id, ['apply_filters' => false]);
        $project = $projects->first();
...
//        $count_all = $this->projectrepo->search('', ['stats' => 'count-all']);
        $count_all = $this->projectrepo->search('', ['stats' => 'count-child']);
