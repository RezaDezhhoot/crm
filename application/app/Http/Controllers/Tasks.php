...
class Tasks extends Controller {
    ...
    public function store(TaskStoreUpdate $request, TaskAssignedRepository $assignedrepo) {
        ...
        $task->supervisors()->attach(\request('supervisor'));
    ...
    public function updateAssigned(TaskAssignedRepository $assignedrepo, $id) {
        ...
        if (\request()->has('supervisors')) {
            $task->supervisors()->sync(
                collect(\request()->input('supervisors'))
                    ->map(function ($v , $k) {
                        return $v == 'on' ? $k : false;
                    })->filter()->toArray()
            );
    ...
    public function recurringSettingsUpdate(TaskRecurrringSettings $request, $id) {
        ...
        //////convert time
        $v = new Verta(request('task_recurring_next'));
        $start_date = $v->formatGregorian('Y-m-d');
        $jalaliDate = Verta::parse($start_date);
        $gregorianDate = $jalaliDate->DateTime();
        $date =  $gregorianDate->format('Y-m-d');
        //////convert time
    ...
    public function managerReports($id)
    {
        $task = \App\Models\Task::Where('task_id', $id)->first();
        $payload = [
            'page' => $this->pageSettings('edit'),
        ...
        $reports = \App\Models\Reports::query()
            ->latest()->where('owner',TaskOwnerEnum::MANAGER)
            ->get();

        //modal request
        if (request('source') == 'modal') {
            $html = view('pages/task/components/manager-reports', compact('task','reports'))->render();
            $jsondata['dom_html'][] = [
                'selector' => '#card-left-panel',
                'action' => 'replace',
                'value' => $html,
    ...
    public function postManagerReports($id)
    {
        $task = \App\Models\Task::Where('task_id', $id)->first();
        $payload = [
            'page' => $this->pageSettings('edit'),
    ...
    public function reports($id)
    {
        ...
        $reports = \App\Models\Reports::query()
            ->latest()->where('owner',TaskOwnerEnum::TEAM)
            ->get();

        //modal request
        if (request('source') == 'modal') {
            $html = view('pages/task/components/reports', compact('task','reports'))->render();
            $jsondata['dom_html'][] = [
                'selector' => '#card-left-panel',
                'action' => 'replace',
                'value' => $html,
    ...
    public function clientReports($id)
    {
        ...
        $reports = \App\Models\Reports::query()
            ->latest()->where('owner',TaskOwnerEnum::CLIENT)
            ->get();

        //modal request
        if (request('source') == 'modal') {
            $html = view('pages/task/components/client-reports', compact('task','reports'))->render();
            $jsondata['dom_html'][] = [
                'selector' => '#card-left-panel',
                'action' => 'replace',
                'value' => $html,
    ...
    public function postClientReports($id)
    {
        $task = \App\Models\Task::Where('task_id', $id)->first();
        $payload = [
            'page' => $this->pageSettings('edit'),
    ...
    public function adminReports($id)
    {
        $task = \App\Models\Task::Where('task_id', $id)->first();
        $payload = [
            'page' => $this->pageSettings('edit'),
        ...
        $reports = \App\Models\Reports::query()
            ->latest()->where('owner',TaskOwnerEnum::ADMIN)
            ->get();

        //modal request
        if (request('source') == 'modal') {
            $html = view('pages/task/components/admin-reports', compact('task','reports'))->render();
            $jsondata['dom_html'][] = [
                'selector' => '#card-left-panel',
                'action' => 'replace',
                'value' => $html,
    ...
    public function postAdminReports($id)
    {
        $task = \App\Models\Task::Where('task_id', $id)->first();
        $payload = [
            'page' => $this->pageSettings('edit'),
