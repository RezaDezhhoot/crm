                    ...
                    <tr data-toggle="collapse" id="item_{{ $child->id }}"  class="accordion-toggle" data-target="#child{{$child->id}}">
                         ...
                         <span class="list-table-action dropdown font-size-inherit">
                                 <!--[delete]-->
                                     <button type="button" title="{{ cleanLang(__('lang.delete')) }}"
                                             ...
                                             data-url="{{ route('admin.skills.delete',$child->id) }}">
                             ...
                             <!--[edit]-->
                                     <button type="button" title="{{ cleanLang(__('lang.edit')) }}"
                                             ...
                                             data-url="{{ route('admin.skills.edit',$child->id) }}"
                                             data-loading-target="commonModalBody" data-modal-title="{{ cleanLang(__('lang.edit_skill')) }}"
                                             data-action-url="{{ route('admin.skills.update',$child->id)  }}" data-action-method="PUT"
                        ...
                        <div>
                            @if($child->children && $child->children->count() > 0)
                                @include('pages.skills.components.table.tree-items',['children' => $child->children,'parent' => $child])
                            @endif
                        </div>
                    </tr>
