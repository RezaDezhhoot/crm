...
<div class="kanban-card kanban-board-element show-modal-button reset-card-modal-form js-ajax-ux-request"
    ...
    <div class="x-meta">
        ...
        <span title="{{ $task->milestone->milestone_title ?? '---' }}">
            <strong>{{ cleanLang(__('lang.milestone')) }}:</strong>
            @if($task->milestone->parent)
                {{ str_limit( $task->milestone->parent->milestone_title ??'---', 68) }} >
            @endif
            {{ str_limit( $task->milestone->milestone_title ??'---', 68) }}
        </span>
        <!--client-->
