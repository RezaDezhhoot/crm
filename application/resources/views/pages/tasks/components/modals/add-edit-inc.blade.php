            ...
            <div class="alert alert-info">
                ...
                <div class="spacer row">
                    <div class="col-sm-12 col-lg-8">
                        <span class="title">{{ cleanLang(__('lang.departments')) }}</span >
                    </div>
                    <div class="col-sm-12 col-lg-4">
                        <div class="switch  text-right">
                            <label>
                                <input type="checkbox" name="show_departments" id="show_departments"
                                       class="js-switch-toggle-hidden-content" data-target="departments_section">
                ...
                <!--description-->
                <div class="hidden" id="departments_section">
                    <div class="form-group row">
                        <div class="col-sm-12">
                            <table class="table w-100 table-bordered" id="departments_section_title">
...
<script>
    $('#assigned').on('change', function (e) {
        var theSelection = $("#assigned").select2("val");
        let jsonData = @json(config('project.assigned'));
        const departments_section_title = document.getElementById('departments_section_title');
        var element = '';

        jsonData.filter(v => {
            return theSelection.includes(v.id.toString());
        }).map(v => {
            if (v.departments.length > 0) {
                element += '<tr>';
                element += '<td>'
                element += v.first_name + " " + v.last_name + ' : '
                element += '</td>'
                v.departments.forEach(item => {
                    element += '<td>'
                    element += item.title
                    element += '</td>'
                })
        ...
        departments_section_title.innerHTML = element;
    });
</script>