@foreach($projects as $project)
    ...
    <tr data-toggle="collapse" data-target="#child{{$project->project_id}}" class="accordion-toggle {{ $project->is_locked ? 'table-primary' : '' }}" id="project_{{ $project->project_id }}">
...
{{--                @endphp--}}
        ...
        <td class="projects_col_project">
            @if(! $project->is_locked)
            <a href="{{ _url('/projects/'.$project->project_id) }}">{{ str_limit($project->project_title ??'---', 80) }} ({{GetCountSubCategory($project->project_id)}}) <a />
                @else
                    <span>
                    {{ $project->project_title }} ({{GetCountSubCategory($project->project_id)}})
    ...
    <tr>
        @include('pages.projects.views.list.table.tree-items',['children' => $project->children,'parent' => $project])
    </tr>
@endforeach